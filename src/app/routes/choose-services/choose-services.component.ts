...
export class ChooseServicesComponent {
  ...
  ngOnInit(): void {
    ...
    this.mealsService.get({quantity:'max'}).subscribe(result =>{
      ...
      console.log('Meallllls ',reservation.items)
  ...
  filterItems(items: Item_Drive[]){
    return items.filter(item => item.sub_category === 5 || item.sub_category === 4 )
  }
  goToNextStep(){
  ...
  total_meals(isAll =false){
    let somme = 0
    const meals = isAll ? (this.meals) : this.filterItems(this.meals)  
  ...
  get_totals(){
    return Number(this.total_meals(true)) + Number(this.total_babyseats()) + Number(this.total_suitcase())
  }
