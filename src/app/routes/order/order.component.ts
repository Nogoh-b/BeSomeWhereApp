...
export class OrderComponent implements OnInit {
  ...
  buy(){
        ...
        this.data.passengers.forEach((passenger: Passenger, index) => {
            if(!passenger.take_at_home && index > 0){
              passenger.birthday = ''
            }
        });
        // console.log(this.data)
    ...
    this.routeService.getRoute(this.data.route_id).subscribe(result =>{
      ...
      if(!this.curRoute.for_disabled && Boolean(this.data.for_disabled)  ){
        ...
        this.toast_c.open('BeSomewhere', 'Plus de place pour handicapé disponible')
        return
      }
      if(this.curRoute.places < this.data.passengers.length ){
        ...
        this.toast_c.open('BeSomewhere', 'Plus assez de place disponible')
        return
      }
      if(!this.reservation.id){
        this.data.status = parseInt(localStorage.getItem('reservation_payment_type'))
        localStorage.removeItem('reservation_payment_type')
        ...
        this.reservationService.post(this.data).subscribe(result =>{
          ...
          this.toast_c.open("BeSomewhere", "reservation_confirmed")
          ...
          const admin_email_content = this.constructEmailMessage('', user.first_name + ' '+user.last_name,user.email,
            ...
            result.id_car,
            'Admin', result.take_at_home,'BesomeWhere',result.passengers[0].flight_number
          )
          //envvoi mail Admin
        ...
        this.reservationService.update(this.data, this.reservation.id).subscribe(result =>{
          ...
          localStorage.removeItem('reservation')
          this.toast_c.open("BeSomewhere", "Reservation Mis à jour")
          // this.route.navigate(['/'], {queryParams:{r:true }})
          setTimeout(() => {
            this.route.navigate(['/'], {queryParams:{r:true }})
          }, 1500);
...
return reservation_t
  ...
  getTotal(){
    ...
    return this.total_price_passenger(this.data.take_at_home != null) + this.total_price_babyseats()+ this.total_price_meals()+ this.total_price_services()+ this.total_price_suitcase()
  }
  get_new_total(){
  ...
  constructEmailMessage(
    ...
    return `
      ...
      <ul>
        ...
        <li><strong>Numéro de vol :</strong> ${flight_number}</li>
  ...
  constructEmailMessageClient(
    ...
    return !take_at_home ? this.emailService.getAdminTextFOrReservation(departureDate, 
      departurePoint, 
      destinationDate, 
      destinationPoint, 
      amount, 
      ...
      take_at_home,
      departurePointTitle,
      destinationPointTitle )  : this.emailService.getAdminTextFOrReservationComeToMyDoor(departureDate, 
        departurePoint, 
        destinationDate, 
        destinationPoint, 
        amount, 
